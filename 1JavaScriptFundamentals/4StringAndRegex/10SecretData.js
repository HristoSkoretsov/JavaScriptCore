function secretData(input) {
    let regexNames = /\*[A-Z][a-zA-Z]*(?=\t| |$)/gm;
    let regexPhones = /\+[0-9\-]{10}(?=\t| |$)/gm;
    let regexId = /![a-zA-Z0-9]+(?=\t| |$)/gm;
    let regexBases = /_[a-zA-Z0-9]+(?=\t| |$)/gm;

    for (let lines of input) {
        console.log(lines
            .replace(regexNames, m => '|'.repeat(m.length))
            .replace(regexPhones, m => '|'.repeat(m.length))
            .replace(regexId, m => '|'.repeat(m.length))
            .replace(regexBases, m => '|'.repeat(m.length))
        );
    }
}
